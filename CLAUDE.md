...
The lens that presupposes no equivariance is the jet-transport definition of §7: *a method
jet-transports iff it is a term in the $C^\infty$-theory generated by evaluation of the field and by
the slot structure of its **shape**, the shape being required to base-change.* Prefer it when
attacking the class directly; the older J1–J4 list is a consequence of it, not an alternative.
...
| Base change **is** the $C^\infty$-ring structure of $A$: J1 is definitional, J2 and J3 are theorems, J4 (the shape) is the whole content | Taylor formula $=$ substitution on the 8 local algebras, $T^A$ functorial on all 17, reciprocal $=$ algebraic inverse, implicit stage transports, shape cocycle (`verify/test_transport.py`) | a genuine prerequisite for transport that is neither implied by being such a term nor a statement about the shape |
