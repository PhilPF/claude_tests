...
The lens that presupposes no equivariance is §7's separation of **transport** from **closure**:
transport is an *operation* — interpret the program over $A$, state ranging over $A^n$ with $n$
unchanged — and it is correct unconditionally; closure is the *coherence equation* comparing it
against the family's own member at dimension $n\dim A$. Prefer that when attacking the class
directly, and do not fuse the two: a definition that did was refuted by the Laplacian method.
...
| Transport is an operation and is unconditionally correct; closure is a separate coherence equation. J1–J3 are prerequisites for transport, J4 is not — it is uniformity of the family in dimension | the Laplacian and aromatic methods transport over all 17 algebras yet close only where $c_A=1_A$ / $\dim A=1$ (`verify/test_transport.py`, T6); base change is the $C^\infty$-ring structure (T1–T4) | a $C^\infty$-typed program whose $A$-interpretation is not $T^A$ of its real one, or a closure failure not traceable to dimension-uniformity |
| The class is finite-dimensional **commutative** algebras, not Weil ones; Weil-ness is exactly what base change of *smooth* data needs | Euler/Heun/midpoint/RK4 are $T^{\mathbb C}$-natural and §4's trichotomy predicts every non-Weil row; $c_A$ is a unit over every split algebra and $0$ over $\mathbb C$, so the Laplacian dies there and lives over $D$ (`verify/test_nonweil.py`) | a closure failure over a non-split algebra not predicted by the trichotomy, or a smooth $f$ with a well-defined $f^{\mathbb C}$ |
| The exact identity $\Delta_{\mathbb R^{nN}}(X^A)=\rho_A(c_A)(\Delta_AX)^A$ holds on every lifted locus, so §4's liana law is a consequence, not a table; $c_{\mathbb C}=0$ *is* harmonicity | verified on nine based algebras, split and non-split (`verify/test_necessary.py`, P3) | a based algebra where the identity fails, or a liana law not recovered from it |
| $\#\operatorname{Hom}(A,\mathbb R)$ is the trace-form **signature** $p-q$; $\mathbb C$ has no augmentation, so (T-base)/(T-fibre) is special to split algebras | direct solve on $A_\lambda$ plus agreement with $\dim A/\mathrm{Nil}=p+q$ exactly on the split ones (`verify/test_necessary.py`, P2) | an algebra whose real-point count differs from $p-q$ |
| Closure over dimension 2 alone is strictly stronger than (T): the defect over $A_\lambda=\mathbb R[x]/(x^2-\lambda)$ is polynomial in $\lambda$, so its zero set is everything or discrete — hence the spectrum is **never** the split algebras | Laplacian defect of $\lambda$-degree 1 vanishing only at $\lambda=0$; $c_{A_\lambda}=1+\lambda$; split $\iff\lambda\ge0$ is neither all nor discrete (`verify/test_necessary.py`, P4–P5) | a method analytic in the jet whose spectrum meets an algebraic family in a non-analytic set |
| Module rigidity is necessary and not sufficient: the output must be $A$-differentiable | Laplacian satisfies it over $\mathbb C$ yet fails closure; marked-coordinate method violates it (`verify/test_necessary.py`, P1) | an algebraically natural method whose output is not $A$-differentiable |
| Decoupling (D) is the multi-field completion of $\mathbb R^k$-closure ($X^{\mathbb R^k}$ is the $k$-fold diagonal), and it is **independent** of closure: leapfrog with the plain halves shape is closed over the whole class and fails (D); the coordinate-wise method decouples and is not closed | nine algebras incl. $\mathbb C$ for leapfrog; $\operatorname{tower}(D)$ fails (D) at three dimension pairs (`verify/test_decoupling.py`) | a closed method failing (D) that is nonetheless affine equivariant, or a proof that closure over $\mathbf{FCA}$ implies (D) |
| The $\operatorname{Der}(A)$ twists $X^A+\delta_A$ are $\pi_A$-related to $X$ and commute with $X^A$; a method keeps the base half and cannot split the pair | $[\delta_A,X^A]=0$ on every algebra with a nonzero derivation; Heun's base half holds, the splitting fails (`verify/test_nonweil.py`) | a derivation with $[\delta_A,X^A]\ne0$, or a consistent method that splits the commuting pair exactly |
