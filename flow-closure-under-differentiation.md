...
The list is still a list of prerequisites, and the next subsection shows it is a list of
prerequisites for **two different things**: J1–J3 are what jet transport needs in order to be
*defined*, while J4 is not about transport at all but about the family's uniformity in dimension.
Read as a single condition it misclassifies the Laplacian and aromatic methods.

### What jet transport is — and why it is not the closure property

**Retraction.** A first version of this subsection defined jet transport as *"a term in the
$C^\infty$-theory generated by field evaluation and the slot structure of a base-changing shape"*.
That is a definition of the wrong thing, and it is refuted by two of this project's own examples: the
**Laplacian method** and the **aromatic method** use a metric and a trace, so that definition denied
them jet transport — yet both are ordinary programs, both run in jet arithmetic, and both return the
correct jet, on all seventeen based algebras (*verified*, `verify/test_transport.py`, T6). The error
was to fuse two statements that must be kept apart.

**Three objects, in order.**

1. **What is transported.** An *$A$-point* of $\mathbb R^n$ is an element of $A^n=T^A\mathbb R^n$.
   For $A=J^r_k=\mathbb R[\delta_1,\dots,\delta_k]/\mathfrak m^{r+1}$ this is exactly an $r$-jet in
   $k$ parameters — a truncated Taylor polynomial $u(\delta)=\sum_{\lvert\alpha\rvert\le r}u_\alpha\delta^\alpha$.
   Jets are the special case; Weil algebras are infinitesimal-neighbourhood data in general.
2. **Transport of a jet along a map** is pushforward: $T^Af=f^A$, given by the finite Taylor sum
   $$f^A(u)=\sum_{\alpha}\frac{\partial^\alpha f(\pi u)}{\alpha!}(u-\pi u)^\alpha,\qquad
   \pi:A\to\mathbb R,\tag{$*$}$$
   finite because $u-\pi u$ is nilpotent, and functorial: $(g\circ f)^A=g^A\circ f^A$. Equivalently,
   $A$ is a **$C^\infty$-ring** — a set with an operation $f_A:A^n\to A^k$ for *every* smooth
   $f$, compatible with composition, identities and products — and $(*)$ is that structure.
   *Verified*: $(*)$ agrees with substitution on the eight local based algebras, and $T^A$ is
   functorial in the realization on all seventeen (T1).
3. **Jet transport of an algorithm** is its *interpretation over $A$*: run the same program with
   $\mathbb R$-arithmetic replaced by $A$-arithmetic, the state ranging over $A^n$ with **$n$
   unchanged**. Write it $P\langle A\rangle$.

> **Theorem (why jet transport works).** If every step of $P$ is a $C^\infty$-ring operation and the
> field is touched only by evaluation, then $P\langle A\rangle=T^A\bigl(P\langle\mathbb R\rangle\bigr)$,
> for every Weil algebra $A$, with no further hypothesis.

*Proof.* Evaluation at an $A$-point is $X^A$ by the definition of base change, and each remaining
step is a $C^\infty$-operation; $(*)$ is functorial, so the composite is $(*)$ of the composite. $\square$

So **transport is an operation, not a property of methods, and its correctness is unconditional once
it is defined.** There is no interesting class of "methods that jet-transport": every
$C^\infty$-typed program does, and being $C^\infty$-typed is a property of the *code*, not of the map
it computes. The prerequisites are prerequisites for $P\langle A\rangle$ to *exist*:

* **J1** field access by evaluation. In practice one never differentiates $X$: the field's own
  program is run on $A$-points and the derivatives fall out of the arithmetic.
* **J2** only $C^\infty$-operations on values. A comparison, a branch, $\min$, $\lvert\cdot\rvert$, a
  tolerance test are not smooth functions and have no interpretation over $A$ — which is why
  implementations branch on the **real part** $\pi$. A norm is not the obstruction here:
  $\lVert v\rVert^2$ is a polynomial. Division is legal exactly on units because $x\mapsto1/x$ is
  smooth exactly there; *verified*, the lift $(*)$ of $1/x$ about $\pi(a)$ is the algebraic inverse
  (T3).
* **J3** is a theorem, not an axiom: if $G$ is smooth with $\partial_zG$ invertible at a real
  solution, the implicit function theorem makes the solution map smooth, hence a $C^\infty$-operation;
  nilpotence makes the Newton lift terminate. *Verified* on the implicit-midpoint stage equation (T4).

**J4 does not belong on that list.** "Auxiliary structure must base-change functorially" is not a
condition for jet arithmetic to run: a metric base-changes perfectly well, and the Laplacian method
transports over every algebra in the family. What J4 was detecting is a property of the *family*,
and it appears only when one compares transport against something else.

**Closure is that comparison.** It is a **coherence condition** relating two ways of growing — raise
the ring at fixed dimension, or raise the dimension over $\mathbb R$ — through the realization
$\iota:\mathbb R^{n\dim A}\to A^n$:

$$\boxed{\ \Psi_{n\dim A}\bigl(\iota^*X^A\bigr)\;=\;\iota^*\bigl(\Psi_n\langle A\rangle\bigr).\ }$$

Jet transport supplies the right-hand side, unconditionally, and is what makes the equation
meaningful at all; **closure is the equation**, and it constrains the family, never the single
member. Every failure §4 measures is a failure of *uniformity in the dimension direction*: at
dimension $n\dim A$ the Laplacian sums $n\dim A$ real slots, at dimension $n$ over $A$ it sums $n$
$A$-slots, and the two realizations agree iff $\sum_\alpha e_\alpha^2=1_A$. *Verified* on all
seventeen based algebras and both methods: transport holds always; closure holds for the Laplacian
exactly where $c_A=1_A$ and for the aromatic method exactly where $\dim_{\mathbb R}A=1$ (T6). §4's
liana and loop laws are thus **derived** here, as uniformity statements rather than a survival table.

**The shape is the bookkeeping of that uniformity**, and belongs to closure rather than to transport.
A *shape system* is a set $\mathbf S$ with a realization $R:\mathbf S\to\mathbf{Mf}$; an operation
$\sigma\mapsto\sigma^A$ with $\sigma^{\mathbb R}=\sigma$ and $(\sigma^B)^A=\sigma^{A\otimes B}$;
identifications $\iota_{\sigma,A}:R(\sigma^A)\to T^A(R\sigma)$ satisfying
$\iota_{\sigma,A\otimes B}=T^A(\iota_{\sigma,B})\circ\iota_{\sigma^B,A}$ under $T^{A\otimes B}=T^AT^B$;
and a class $\mathfrak X_\sigma$ of admissible fields with $X\in\mathfrak X_\sigma\Rightarrow
X^A\in\mathfrak X_{\sigma^A}$. Three items this project had already met are these axioms:

* the **$\otimes$-monoid** structure of the closure spectrum *is* the cocycle. *Verified*:
  $T^{A\otimes B}=T^AT^B$ on the nose in the block realization, the slot identity
  $jN_C+(\beta N_A+\alpha)=(jN_B+\beta)N_A+\alpha$, and leapfrog over $D\otimes D$ closed for the
  twice-lifted splitting and for neither of two alternatives (T5);
* $\iota$ is the standing trap ("distrust any statement that silently fixes
  $\mathbb R^{n\dim A}\cong A^n$") promoted to a datum;
* §9's **crossed $T^*$ transport** is stability of the admissible class — leapfrog's is *separable
  Hamiltonian*, separable exactly under the crossed splitting — so that finding is an instance of an
  axiom rather than an exception.

**Where this leaves the open problem.** Not at "is every closed method a program?", which the theorem
above makes the wrong question: programs transport, and transport is not closure. It splits instead as

1. *which dimension-indexed structures are uniform* — §4 answers it for tensors, and T6 shows the
   answer is exactly the trichotomy;
2. *must a closed family come from a dimension-indexed rule at all?* — the extension theorem says no
   in general, and the junk it manufactures is precisely the residue.

One observation the split makes visible, and it is the honest form of the recorded factorisation
$[\text{equivariance}]\cap[\text{closure}]$: a uniform rule built from $u$, $h$ and field evaluations
**with no auxiliary structure at all** has nothing left that could fail to be $GL$-equivariant, and
that is the B-series world; leapfrog and the Laplacian method are exactly the cases where the rule
carries structure — a splitting, uniform for every $A$; a metric, uniform only when $c_A=1$.
...
* E. J. Dubuc, *$C^\infty$-schemes*, Amer. J. Math. **103** (1981) 683–690; I. Moerdijk, G. E. Reyes, *Models for Smooth Infinitesimal Analysis*, Springer 1991; D. Joyce, *Algebraic Geometry over $C^\infty$-rings*, Mem. AMS **260** (2019). (Cited from background — see `references/literature.md`, provenance **[B]**.)
