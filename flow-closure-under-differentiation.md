...
The list is still a list of prerequisites. The next subsection replaces it by a definition, in which
J1 is definitional, J2 and J3 are theorems, and J4 is the whole content.

### The definition: jet transport is the $C^\infty$-ring semantics

J1–J4 is a list of prerequisites, not a definition, and it collapses once the structure it was
circling is named.

**A Weil algebra is a $C^\infty$-ring, and base change is its $C^\infty$-structure.** A
$C^\infty$-ring is a set $R$ with an operation $f_R:R^n\to R^k$ for *every* smooth
$f:\mathbb R^n\to\mathbb R^k$, compatible with composition, identities and products: the algebraic
theory whose operations are all smooth maps, not just $+$ and $\times$. For a Weil algebra that
structure exists, is unique, and is given by
$$f^A(u)\;=\;\sum_{\alpha\in\mathbb N^n}\frac{\partial^\alpha f(\pi u)}{\alpha!}\,(u-\pi u)^\alpha,
\qquad \pi:A\to\mathbb R\ \text{the augmentation},\tag{$*$}$$
a **finite** sum, because $u-\pi u$ lies in the nilpotent ideal — and $(*)$ is precisely $T^Af$. For
$A=\prod_iA_i$ apply $(*)$ in each factor. *Verified*: $(*)$ agrees with substitution on every local
based algebra of the family, and $T^A$ is functorial in the realization on all seventeen
(`verify/test_transport.py`, T1).

So "run the program in $A$-arithmetic" is not a metaphor. It is **interpretation in the
$C^\infty$-ring $A$**, and jet transport is the naturality of that interpretation. Read against the
list, three of the four items stop being hypotheses:

* **J1 is definitional.** Evaluation at an $A$-point *is* $f^A$.
* **J2 splits in two, and had been conflating them.** Every *smooth* operation is admissible, so
  what J2 really excludes is non-smooth syntax — a comparison, a branch, $\min$, $\lvert\cdot\rvert$.
  A **norm** is not excluded by smoothness at all ($\lVert v\rVert^2$ is a polynomial); it is
  excluded by J4. And division is legal exactly on units not by fiat but because $x\mapsto1/x$ is
  smooth exactly there. *Verified*: the lift $(*)$ of $1/x$ about $\pi(a)$ equals the algebraic
  inverse $a^{-1}$, on every local algebra of the family (T3).
* **J3 is a theorem, not an axiom.** If $G$ is smooth and $\partial_zG$ invertible at a real
  solution, the implicit function theorem makes the local solution map smooth, hence a
  $C^\infty$-operation, hence transported by functoriality; nilpotence is what makes the Newton
  lift terminate. *Verified*: the implicit-midpoint stage equation satisfies $z^A=z_A$ on six
  algebras, and the full step on three (T4).
* **J4 survives, and is the entire remaining content.** It is not about $C^\infty$-rings.

What is left is one sentence.

> **Definition (jet transport).** A method **jet-transports** if it is a term in the
> $C^\infty$-theory generated by (a) evaluation of the field and finitely many of its derivatives at
> computed points, and (b) the slot structure of its shape — the shape being required to
> base-change.

Clause (a) is the standing locality hypothesis of §1 in syntactic form: finitely many derivatives at
computed points is exactly what Peetre's theorem forces. Clause (b) is J4.

**Why (b) is not vacuous, and why it explains §4.** The Euclidean sum $\sum_{i=1}^{d}u_i^2$ is a
perfectly good term at every dimension $d$, yet the Laplacian method built from it is not closed.
The definition says why, where measurement only said *that*: at the shape $\sigma^A$ the term sums
$nN$ **real** slots, whereas transport asks for a sum over $n$ slots **valued in $A$**. The two agree
iff $\sum_\alpha e_\alpha^2=1_A$ — the invariant $c_A$ — so §4's trichotomy is not an empirical
table but the computation of which dimension-indexed structures are shapes. A splitting has no such
defect: a subset $I\subseteq\{1,\dots,n\}$ of slots base-changes to the union of its blocks. That is
the whole difference between leapfrog and the Laplacian method.

**The shape, precisely.** A *shape system* is
1. a set $\mathbf S$ with a realization $R:\mathbf S\to\mathbf{Mf}$;
2. for each Weil algebra $A$ an operation $\sigma\mapsto\sigma^A$ on $\mathbf S$, with
   $\sigma^{\mathbb R}=\sigma$ and $(\sigma^B)^A=\sigma^{A\otimes B}$;
3. isomorphisms $\iota_{\sigma,A}:R(\sigma^A)\to T^A(R\sigma)$ with $\iota_{\sigma,\mathbb R}=\mathrm{id}$
   and the cocycle $\iota_{\sigma,A\otimes B}=T^A(\iota_{\sigma,B})\circ\iota_{\sigma^B,A}$, under
   $T^{A\otimes B}=T^AT^B$;
4. a class $\mathfrak X_\sigma\subseteq\mathfrak X(R\sigma)$ of admissible fields, **stable**:
   $X\in\mathfrak X_\sigma\Rightarrow X^A\in\mathfrak X_{\sigma^A}$.

> **Definition (transport, semantic form).** $\Psi$ *transports over $\mathbf S$* if for every
> $\sigma\in\mathbf S$, every Weil algebra $A$, every $X\in\mathfrak X_\sigma$ and every small real $h$,
> $$\iota_{\sigma,A}\circ\Psi^{\,\iota_{\sigma,A}^*X^A}_{\sigma^A,\,h}\circ\iota_{\sigma,A}^{-1}
> \;=\;T^A\bigl(\Psi^X_{\sigma,h}\bigr).$$

Plain dimension is the trivial shape ($\mathbf S=\mathbb N$, $n^A=n\dim A$, $\iota$ the block
convention, all fields admissible); leapfrog's is dimension-with-a-splitting. Three of the four
axioms are things this project had already met and not named:

* **2–3 are why the closure spectrum is a $\otimes$-monoid.** "Closed over $A$ and over $B$ implies
  closed over $A\otimes B$" is the cocycle, not a coincidence. *Verified*: $T^{A\otimes B}=T^AT^B$
  holds on the nose in the block realization, the slot identity
  $jN_C+(\beta N_A+\alpha)=(jN_B+\beta)N_A+\alpha$ holds, leapfrog's splitting lifted twice by $D$ is
  exactly the $D\otimes D$ splitting, and leapfrog is closed for it and for neither of two
  alternatives (T5).
* **3 is the standing trap promoted to a datum.** "Distrust any statement that silently fixes an
  identification $\mathbb R^{n\dim A}\cong A^n$" is the instruction not to leave $\iota$ implicit.
* **4 is what §9's $T/T^*$ asymmetry is a statement about.** Leapfrog's admissible class for reverse
  mode is *separable Hamiltonian*, and the verified finding that the shape transports **crossed**
  under $T^*$ while blockwise under $T$ says exactly that axiom 4 holds for the crossed transport
  and fails for the blockwise one. The anomaly is an instance of an axiom, not an exception to a
  rule.

**Soundness, and where the definition stops.** A term is interpreted by $C^\infty$-operations, which
commute with base change by functoriality, so *term $\Rightarrow$ transports*. The converse is
false as stated: the extension theorem manufactures closed families by gluing, and those are terms in
no interface. So the definition is a **sufficient** condition of the right shape, and the open
problem is exactly

$$\text{is every closed method a term?}\qquad\text{gap}=\text{the extension-theorem junk.}$$

Two further points the definition makes visible. First, **the step size**: in a term $h$ is a
constant symbol, interpreted over $A$ as $h\cdot1_A$, which is why the property is correctly stated
with **real** $h$; promoting $h$ to a slot — transporting the step size too — is automatic for terms
and a priori stronger for closed families, and this project does not settle whether it is strictly
stronger. Second, **the expansion point in $(*)$ is forced, and there is one per local factor**:
for polynomial $f$ Taylor's theorem is exact about any point, so the datum is invisible, but for
genuinely smooth $f$ the series terminates only when $u-a$ is nilpotent, i.e. $a=\pi(u)$ in each
factor. *Verified*: $\dim_{\mathbb R}A/\mathrm{Nil}(A)$, computed as the corank of the trace form,
counts those points — $1$ for the eight local algebras of the family, $2$–$4$ for the nine products —
and in $\mathbb R^2$ at $u=(0,1)$ no real $c$ makes $u-c\cdot1$ nilpotent, since nilpotent in
$\mathbb R^k$ means componentwise zero (T2 checks $c=0,1,2$; the general case is that one line).
That is the trap behind gap 3 above, stated as a property of the definition rather than of one
witness.

Finally, this places Gimeno et al. Their jet transport is this definition with $A=J^r_k$ the
truncated-polynomial algebra and $\mathbf S$ the trivial shape; the programme here is the same
construction with **both** the algebra and the shape made variable.
